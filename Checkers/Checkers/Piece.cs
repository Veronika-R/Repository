...
namespace Checkers
{
    public class Piece:IPiece
    {
        ...
        public IEnumerable<Position> Jumps(Game game, Position position)
        {
            var jumps = new List<Position>();

            var targetPositions = GetJumpDirection(Position.X, Position.Y);

            foreach (var targetPos in targetPositions)
            {
                // Look for an opposing piece
                if (game.IsCaptured(targetPos,Color))
                {
                    // Found one! Look for a valid space *beyond* this piece
                    var potentialJump = PotentialJump(position, targetPos);

                    if (game.CheckWalls(potentialJump) && game.PieceAt(potentialJump) == null)
                    {
                        jumps.Add(potentialJump);
                    }
                }
            }
            return jumps;
